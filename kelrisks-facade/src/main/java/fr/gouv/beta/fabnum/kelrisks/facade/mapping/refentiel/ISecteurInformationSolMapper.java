...
public interface ISecteurInformationSolMapper extends IGeometryMapper {
    ...
    //    @formatter:off
    @Mappings(
            @Mapping(target = "ewkt", expression = "java(GeoJsonUtils.toGeoJson(secteurInformationSol.getMultiPolygon(), Stream.of(new AbstractMap.SimpleEntry<>(\"Numéro\", StringUtils.stripToEmpty(secteurInformationSol.getNumero()))).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue))))")
    )
    //    @formatter:on
