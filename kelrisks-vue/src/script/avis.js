export default {
  ...
  getBasiasParcelle (value) {
    ...
    if (avis.numberOf === 0) {
      ...
      avis.lib = 'est référencée dans l\'inventaire des sites ayant accueilli par le passé une activité susceptible d\'avoir pu généré une pollution des sols (BASIAS). + lien vers fiche Basias'
      value.entity.siteIndustrielBasiasSurParcelleDTOs.forEach(function (element) {
        avis.lib += element.identifiant + ', '
      }, this)
    }
    return avis
  },
  ...
  getICSurParcelle (value) {
    ...
    if (avis.numberOf === 0) {
      ...
      value.entity.installationClasseeSurParcelleDTOs.forEach(function (element) {
        avis.lib += element.raisonSociale + ', '
      }, this)
    }
    return avis
  },
